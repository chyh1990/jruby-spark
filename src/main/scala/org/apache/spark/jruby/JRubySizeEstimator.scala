...
import org.jruby.{Ruby, RubyModule}

import scala.collection.mutable.ArrayBuffer
import scala.runtime.ScalaRunTime


// mostly copied from SizeEstimator.scala
// only black list some global references in JRuby objects
object JRubySizeEstimator extends Logging {
  ...
  // def estimate(obj: IRubyObject): Long = estimate(obj, new util.IdentityHashMap[IRubyObject, IRubyObject])
  ...
  private def visitSingleObject(obj: AnyRef, state: SearchState) {
    ...
    } else if (obj.isInstanceOf[RubyModule]
      || obj.isInstanceOf[Ruby]
      || obj.isInstanceOf[ThreadContext]) {
      // black list some jruby global reference
      // println("BLACK " + obj.getClass.getName)
    } else {
      obj match {
  ...
  // JRuby object array
  private val ARRAY_SIZE_FOR_SAMPLING = 100
  private val ARRAY_SAMPLE_SIZE = 25 // should be lower than ARRAY_SIZE_FOR_SAMPLING
  ...
  private def sampleArray(
    ...
    for (i <- 0 until ARRAY_SAMPLE_SIZE) {
      ...
      if (obj != null) {
        size += JRubySizeEstimator.estimate(obj, state.visited).toLong
      }
    }
    size
  }
